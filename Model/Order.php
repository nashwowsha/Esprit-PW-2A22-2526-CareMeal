...
class Order
{
    ...
    public function all(?string $userId = null): array
    {
        if ($userId !== null && $userId !== '') {
            $sql = "SELECT o.*, p.nom AS product_name, p.prix_commande AS product_price, 'CareMeal' AS partnerName FROM commande o LEFT JOIN produit p ON o.id_produit = p.id_produit WHERE o.id_user = :uid ORDER BY o.date_commande DESC";
            $stmt = $this->pdo->prepare($sql);
            $stmt->execute([':uid' => $userId]);
        } else {
            $sql = "SELECT o.*, p.nom AS product_name, p.prix_commande AS product_price, 'CareMeal' AS partnerName FROM commande o LEFT JOIN produit p ON o.id_produit = p.id_produit ORDER BY o.date_commande DESC";
    ...
    {
        $stmt = $this->pdo->prepare("SELECT o.*, p.nom AS product_name, p.prix_commande AS product_price, 'CareMeal' AS partnerName FROM commande o LEFT JOIN produit p ON o.id_produit = p.id_produit WHERE o.id_commande = :id LIMIT 1");
        $stmt->execute([':id' => $id]);
        $row = $stmt->fetch();
        return $row ?: null;
    }
