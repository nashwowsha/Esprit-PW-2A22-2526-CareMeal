...
class PlanningCollecteController
{
    ...
    private function buildItemsFingerprint($items)
    {
        ...
        ksort($map);
        $json = json_encode($map, JSON_UNESCAPED_UNICODE | JSON_INVALID_UTF8_SUBSTITUTE);
        return is_string($json) ? $json : '';
    }

    private function findPendingDuplicateCollecteFromSource($source)
    {
        ...
        if ($idRestaurant <= 0 || $idPref <= 0 || $idUser <= 0) {
            return 0;
        }
        if (!in_array($mode, ['pickup', 'delivery'], true)) {
            return 0;
        }
        if ($requestedTime === '' || !is_array($items)) {
            return 0;
        }

        $incomingFingerprint = $this->buildItemsFingerprint($items);
        ...
        try {
            $query = $this->db()->prepare(
                "SELECT id_collecte, items_json
                 FROM planning_collecte
                 WHERE id_restaurant = :id_restaurant
                   AND id_pref = :id_pref
                   AND id_user = :id_user
                   AND mode_collecte = :mode_collecte
                   AND statut = 'en_attente'
                   AND DATE(heure_souhaitee) = CURDATE()
                   AND TIME(heure_souhaitee) = :heure
                   AND created_at >= DATE_SUB(NOW(), INTERVAL 6 HOUR)
                 ORDER BY id_collecte DESC"
            );
            $query->execute([
        ...
        foreach ($rows as $row) {
            $existingItems = json_decode((string)($row['items_json'] ?? '[]'), true);
            if (!is_array($existingItems)) {
                continue;
            }
            $existingFingerprint = $this->buildItemsFingerprint($existingItems);
            if ($existingFingerprint !== '' && hash_equals($incomingFingerprint, $existingFingerprint)) {
                return (int)($row['id_collecte'] ?? 0);
            }
        }
    ...
    {
        $query = $this->db()->prepare("SELECT id FROM users WHERE id = :id LIMIT 1");
    ...
    public function createCollecte($source)
    {
        ...
        // Idempotency guard: if same pending collecte already exists, reuse it.
        $duplicateCollecteId = $this->findPendingDuplicateCollecteFromSource($source);
        if ($duplicateCollecteId > 0) {
            return [
                'ok' => true,
                'status' => 'success_collecte_created',
                'id_collecte' => $duplicateCollecteId,
            ];
        }
    ...
    public function getPartnerCollectesWithJoin($idOwner, $idRestaurant = 0, $search = '', $sortBy = 'newest')
    {
        ...
        if ($search !== '') {
            $where .= " AND (
                ...
                OR LOWER(COALESCE(NULLIF(TRIM(CONCAT(COALESCE(up.prenom, ''), ' ', COALESCE(up.nom, ''))), ''), u.email, CONCAT('user #', u.id))) LIKE LOWER(:q)
                OR CAST(pc.id_collecte AS CHAR) LIKE :q
                OR CAST(pc.id_user AS CHAR) LIKE :q
            )";
            $params['q'] = '%' . $search . '%';
