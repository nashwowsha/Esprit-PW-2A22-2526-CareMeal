...
// ⚠️  Ne jamais committer la clé — la mettre dans config/api_keys.php
$openai_key = defined('GROQ_API_KEY') ? GROQ_API_KEY : 'VOTRE_CLE_GROQ_ICI';
...
try {
    ...
    $stmtEv = $conn->prepare("
        SELECT e.titre, e.date_evenement, e.heure_debut, e.heure_fin,
               ...
               (SELECT COUNT(*) FROM participation p WHERE p.nom_evenement = e.titre AND p.statut != 'Annulé') as inscrits
    ...
    if ($user_role === 'student') {
        ...
        $parts = $conn->prepare("SELECT nom_evenement, date_inscription, statut FROM participation WHERE email = :email ORDER BY date_inscription DESC");
        $parts->bindParam(':email', $student_email);
        $parts->execute();
        ...
        $pev = $conn->prepare("SELECT e.titre, e.date_evenement, e.statut_validation, e.motif_refus, e.capacite_max, (SELECT COUNT(*) FROM participation p WHERE p.nom_evenement = e.titre AND p.statut != 'Annulé') as inscrits FROM EVENEMENT e WHERE e.createur_type = 'Partenaire' AND e.createur_id = :id ORDER BY e.date_evenement DESC");
