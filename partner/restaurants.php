...
// Utiliser l'id de session en priorité, GET en fallback
$idOwner = !empty($_SESSION['user_id']) ? (int)$_SESSION['user_id'] : (isset($_GET['id_owner']) ? (int)$_GET['id_owner'] : 0);
if ($idOwner <= 0) {
    $idOwner = isset($_GET['id_owner']) ? (int)$_GET['id_owner'] : 0;
}
$status = $_GET['status'] ?? '';
...
<head>
  ...
  <link rel="stylesheet" href="<?= htmlspecialchars(caremeal_path('assets/vendor/leaflet/leaflet.css'), ENT_QUOTES, 'UTF-8') ?>">
  <style>
...
<body>
    ...
    <aside class="sidebar" id="sidebar">
      ...
      <nav class="sidebar-nav">
        <div class="sidebar-section">
          ...
          <a href="<?= htmlspecialchars(caremeal_path('partner/restaurants.php'), ENT_QUOTES, 'UTF-8') ?>" class="sidebar-link active"><span class="link-icon"><i class="fa-solid fa-utensils"></i></span> Mes Restaurants</a>
  ...
  <script src="<?= htmlspecialchars(caremeal_path('assets/vendor/leaflet/leaflet.js'), ENT_QUOTES, 'UTF-8') ?>"></script>
  <?php if (!empty($realtimeConfig['enabled'])): ?>
    <script src="https://js.pusher.com/8.4.0/pusher.min.js"></script>
  <?php endif; ?>
  <script src="<?= htmlspecialchars(caremeal_path('js/collecte-address-picker.js'), ENT_QUOTES, 'UTF-8') ?>"></script>
  <script src="<?= htmlspecialchars(caremeal_path('js/collectes-map.js'), ENT_QUOTES, 'UTF-8') ?>"></script>
  <script src="<?= htmlspecialchars(caremeal_path('js/partner-restaurants-validation.js'), ENT_QUOTES, 'UTF-8') ?>"></script>
  <script>
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
      ...
      const phpOwnerId = '<?= (int)$idOwner ?>';
      if (phpOwnerId) {
        document.querySelectorAll('.js-owner-id').forEach((el) => { el.value = phpOwnerId; });
      }
