...
$sessionPartnerId = $_SESSION['user_id'] ?? '';
...
<head>
  ...
  <style>
    ...
    select.form-input option { background-color: #ffffff; color: #0f172a; }
    select.form-input { background-color: #ffffff; color: #0f172a; border-color: #d1d5db; }
    ...
    .filter-chips { display: flex; gap: 8px; flex-wrap: wrap; flex: 1; align-items: center; }
    .filter-chip {
    ...
    .filter-chip:hover { border-color: #f97316; color: #9a3412; background: #ffedd5; }
    .filter-chip.active { background: #fff7ed; border-color: #f97316; color: #ea580c; }
    .filter-chip:not(.active) .chip-dot { opacity: .45; }
    ...
    .form-card .input-icon { color: #94a3b8; }
    .cat-pill-name { color: #ffffff; }
    #partner-filter-cat-list .filter-chip { color: inherit !important; }

    /* Champs clairement visibles (fond blanc + texte sombre) */
    .form-card .form-input,
    .form-card .form-textarea,
    .form-card select.form-input {
    ...
    .form-card .input-icon { color: #64748b !important; }
    .cat-pill-label {
      background: #ffffff !important;
      border-color: #d1d5db !important;
      color: #0f172a !important;
    }
    .cat-pill-label .cat-pill-name,
    ...
    .cat-pill-label:has(input:checked) .cat-pill-name,
...
<div class="dashboard-layout">
  ...
  <aside class="sidebar" id="sidebar">
    <div class="sidebar-header">
      <div class="sidebar-logo"><img src="<?= htmlspecialchars(caremeal_path('assets/logo.png'), ENT_QUOTES, 'UTF-8') ?>" alt="Logo" style="max-width:100%;max-height:100%;object-fit:contain;"></div>
      <div class="sidebar-brand">Care<span>Meal</span></div>
    </div>
    <nav class="sidebar-nav">
      <div class="sidebar-section">
        <div class="sidebar-section-title">Partenaire</div>
        <a href="<?= htmlspecialchars(caremeal_path('View/FrontOffice/partner/dashboard.php'), ENT_QUOTES, 'UTF-8') ?>" class="sidebar-link"><span class="link-icon"><i class="fa-solid fa-store"></i></span> Mon Établissement</a>
        <a href="<?= htmlspecialchars(caremeal_path('partner/offers.php'), ENT_QUOTES, 'UTF-8') ?>" class="sidebar-link active"><span class="link-icon"><i class="fa-solid fa-bag-shopping"></i></span> Mes Offres</a>
        <a href="<?= htmlspecialchars(caremeal_path('View/FrontOffice/partner/events.php'), ENT_QUOTES, 'UTF-8') ?>" class="sidebar-link"><span class="link-icon"><i class="fa-solid fa-calendar-day"></i></span> Mes Événements</a>
        ...
        <a href="<?= htmlspecialchars(caremeal_path('View/FrontOffice/partner/settings.php'), ENT_QUOTES, 'UTF-8') ?>" class="sidebar-link"><span class="link-icon"><i class="fa-solid fa-gear"></i></span> Paramètres</a>
      </div>
    </nav>
      ...
      <div id="view-list">
        ...
        <div class="grid grid-2 gap-4 mb-8 animate-fade-in-up">
          <div class="card" style="display:flex;align-items:center;gap:16px;">
            ...
            <div>
              <div style="font-size:1.5rem;font-weight:700;color:#000000;"><?= (int)($counts['publiée'] ?? 0) ?></div>
        ...
        <div id="form-create-wrap" style="display:none;">
          <div class="form-card">
            <form method="POST" action="offers.php" id="form-create">
              ...
              <input type="hidden" name="id_partenaire" id="create-id_partenaire" value="<?= e($sessionPartnerId) ?>">
...
/* -- Injection id partenaire depuis la session PHP -- */
(function injectPartnerId() {
  const phpPartnerId = '<?= e($sessionPartnerId) ?>';
  if (!phpPartnerId) return;
  const el = document.getElementById('create-id_partenaire');
  if (el && !el.value) el.value = phpPartnerId;
})();
