...
const Admin = {
  ...
  async loadDashboard() {
    ...
    try {
      const res = await fetch('/projet2a22/Controller/UserController.php', {
        method: 'POST',
  ...
  async loadUsers() {
    ...
    if(!user) return;
    Components.confirm(ban ? "Bannir l utilisateur" : "R�activer l utilisateur", "Voulez-vous " + action + " <strong>" + user.name + "</strong> ?", async () => {
      
      try {
        await fetch('/projet2a22/Controller/UserController.php', { method: "POST", headers: {"Content-Type":"application/json"}, body: window.JSON.stringify({action: "update_user_status", user_id: userId, status: ban ? "banned" : "active"}) });
        App.updateUser(userId, { status: ban ? "banned" : "active" });
        Components.showToast("Succ�s", user.name + " a �t� " + (ban ? "bannir" : "r�activ�"), ban ? "warning" : "success");
    ...
    const user = App.getUserById(userId) || App.getUsers().find(u => u.id == userId);
    if(!user) return;
    Components.confirm("Supprimer l utilisateur", "Cette action est irr�versible. Supprimer <strong>" + user.name + "</strong> ?", async () => {
      
      try {
        await fetch('/projet2a22/Controller/UserController.php', { method: "POST", headers: {"Content-Type":"application/json"}, body: window.JSON.stringify({action: "delete_user", user_id: userId}) });
        App.deleteUser(userId);
        Components.showToast("Supprim�", user.name + " a �t� supprim�.", "error");
        if(typeof Admin !=="undefined" && Admin.loadUsers) Admin.loadUsers(); else this.loadUsers();
      } catch(e) { console.error(e); }
    });
  },
  ...
  loadPartners() {
    ...
    validatePartner(partnerId, approve) {
      ...
      Components.confirm(approve ? "Valider le partenaire" : "Refuser le partenaire", "Voulez-vous " + action + " <strong>" + partner.name + "</strong> ?", async () => {
        ...
        try {
          await fetch('/projet2a22/Controller/UserController.php', { method: "POST", headers: {"Content-Type":"application/json"}, body: window.JSON.stringify({ action: "update_user_status", user_id: partnerId, status: approve ? "active" : "banned" }) });
          App.updateUser(partnerId, { status: approve ? "active" : "banned" });
          Components.showToast("Succ�s", partner.name + " a �t� " + (approve ? "valid�" : "refus�"), approve ? "success" : "warning");
