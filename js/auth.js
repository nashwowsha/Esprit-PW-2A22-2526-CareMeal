...
const Auth = {
  ...
  handleLogin(e) {
    ...
    const validation = Validation.validateForm({
      'login-email': { required: true, email: true, name: 'Email' },
      'login-password': { required: true, minLength: 1, name: 'Mot de passe' }
    });
    ...
    if (!user) {
      Components.showToast('Erreur', 'Aucun compte trouvé avec cet email', 'error');
      return;
    }

    if (user.password !== password) {
      Components.showToast('Erreur', 'Mot de passe incorrect', 'error');
      return;
    }

    if (user.status === 'banned') {
      Components.showToast('Erreur', 'Votre compte a été suspendu. Contactez le support.', 'error');
      return;
    }

    if (user.status === 'pending') {
      Components.showToast('Info', 'Votre compte est en attente de validation par un administrateur.', 'warning');
      return;
    }
