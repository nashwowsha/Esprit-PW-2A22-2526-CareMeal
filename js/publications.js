const Publications = {
  ...
  getAuthorName(auteurType, auteurId) {
    ...
    if (auteurId !== undefined && auteurId !== null && String(auteurId).trim() !== '' && String(auteurId) !== '0') {
      const found = App.getUserById(String(auteurId));
    ...
    if (
      currentUser &&
      String(currentUser.id) === String(auteurId) &&
      currentUser.role === role
    ) {
      return currentUser.name;
    }

    // 3. Fallback propre selon le rôle (jamais d'ID brut affiché)
  ...
  async fetchAllReactions() {
    try {
      const response = await fetch('/webEsprit/Controller/PublicationController.php?action=list_reactions');
  ...
  async fetchAllCommentReactions() {
    try {
      const response = await fetch('/webEsprit/Controller/CommentController.php?action=list_reactions');
  ...
  async addPublication(content, imageFile = null) {
    ...
    try {
      ...
      Components.showToast('Erreur', result.message || 'La publication n’a pas pu être enregistrée.', 'danger');
      return false;
    } catch (error) {
  ...
  normalizeImageUrl(rawPath) {
    ...
    if (value.startsWith('/webEsprit/')) return value;
    if (value.startsWith('webEsprit/')) return `/${value}`;
    return `/webEsprit/${value.replace(/^\/+/, '')}`;
  },
      ...
      <div class="pub-compose" id="pub-compose-area">
        ...
        <div class="pub-compose-body">
          ...
          <div class="pub-compose-input-area" style="width: 100%;">
            ...
            <div class="pub-compose-actions" style="display:flex; justify-content:space-between; align-items:center; margin-top: 8px;">
  ...
  async handleReact(pubId, type) {
    ...
    try {
      ...
      const response = await fetch('/webEsprit/Controller/PublicationController.php?action=react', {
        method: 'POST',
        body: formData
      });
  ...
  async handleCommentReact(pubId, commentId, type) {
    ...
    try {
      ...
      const response = await fetch('/webEsprit/Controller/CommentController.php?action=react', {
        method: 'POST',
        body: formData
      });
  ...
  openEditPage(pubId) {
    ...
    if (this.isAdminArea()) {
      window.location.href = `publication-edit.html?id=${encodeURIComponent(pubId)}`;
      return;
    }
  ...
  async fetchStats() {
    try {
      const res = await fetch('/webEsprit/Controller/PublicationController.php?action=stats');
        ...
        <div id="pub-stats-body">
          <div class="pub-stats-grid">
            ...
            <div class="pub-stat-card">
              ...
              <div class="pub-stat-label">Commentaires total</div>
            </div>
            <div class="pub-stat-card">
  ...
  openAdminCreatePage() {
    window.location.href = 'publication-create.html';
  },
