const EventsPartner = {
    ...
    normalizedStatus(rawStatus) {
        ...
        if (status.includes('valid') || status.includes('planifi')) return 'Validé / Planifié';
    ...
    renderStats() {
        ...
        const validated = this.events.filter(e => this.normalizedStatus(e.status || e.statut_validation) === 'Validé / Planifié').length;
    ...
    renderList() {
        ...
        this.events.forEach(ev => {
            ...
            if (normalizedStatus === 'En attente') {
                ...
                `;
                bottomWarning = '<div style="margin-top:12px; font-size:0.8rem; color:var(--color-warning); text-align:center;"><i class="fa-solid fa-triangle-exclamation"></i> ?? Votre événement n\'est pas encore visible...</div>';
            } else if (normalizedStatus === 'Validé / Planifié') {
                buttonsHtml = `
                    <button class="btn btn-secondary btn-sm" onclick="EventsPartner.showDetails(${ev.id})"><i class="fa-solid fa-eye"></i> Voir</button>
                    <button class="btn btn-primary btn-sm" onclick="EventsPartner.editEvent(${ev.id})"><i class="fa-solid fa-pen"></i> Modifier</button>
                    <button class="btn btn-danger btn-sm" onclick="EventsPartner.cancelEvent(${ev.id})"><i class="fa-solid fa-trash"></i> Annuler</button>
