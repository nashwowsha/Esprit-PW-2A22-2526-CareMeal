const StudentVoiceAssistant = {
  ...
  async transcribeWithServerStt(blob) {
    ...
    try {
      const response = await fetch(this.getApiUrl("/api/student-stt.php"), {
        method: "POST",
        body: form,
      });
  ...
  handleStudentNavigation(normalizedText) {
    const routes = [
      ...
      { words: ["profil", "profile"], url: "profile.php", msg: "J'ouvre ton profil." },
      { words: ["preference", "preferences"], url: "preferences.php", msg: "J'ouvre les preferences." },
      { words: ["participation", "participations", "inscription", "inscriptions", "mes inscriptions"], url: "events.php?view=my", msg: "J'ouvre tes participations." },
      { words: ["evenement", "evenements", "event"], url: "events.php?view=all", msg: "J'ouvre les evenements." },
      { words: ["commande", "commandes", "orders"], url: "orders.php", msg: "J'ouvre les commandes." },
      { words: ["fil actualite", "fil d actualite", "actualite", "feed"], url: "View/FrontOffice/feed.php", msg: "J'ouvre le fil d'actualite." },
      { words: ["parametre", "parametres", "settings"], url: "settings.php", msg: "J'ouvre les parametres." },
      { words: ["point", "points"], url: "points.php", msg: "J'ouvre les points." },
    ];
  ...
  autoBriefing() {
    ...
    const user = (typeof App !== "undefined" && typeof App.getCurrentUser === "function")
      ? App.getCurrentUser()
      : null;
    const userId = String(user?.id || user?.user_id || "student");
    const onceKey = `caremeal_student_briefing_once_${userId}`;
  ...
  extractKeyword(normalizedText) {
    const cleaned = normalizedText
      ...
      .replace(/categories?\s+(?:de|du|des|d)?\s*[Â«"']?\s*[a-z0-9\s-]+\s*[Â»"']?/g, " ")
  ...
  extractCategoryName(normalizedText) {
    const m = normalizedText.match(/categories?\s+(?:de|du|des|d)?\s*[Â«"']?\s*([a-z0-9\s-]+?)\s*[Â»"']?(?:\s|$)/);
    if (!m) return "";
    return (m[1] || "")
      .trim()
  ...
  applyMaxPriceFilter(maxPrice) {
    for (const card of this.getAllOfferCards()) {
      ...
      // "moins de X" doit etre strictement inferieur a X
  ...
  getApiUrl(path) {
    ...
    if (typeof window.caremealPath === "function") {
      return window.caremealPath(cleanPath);
    }

    const pathname = String(window.location.pathname || "");
    const marker = "/View/FrontOffice/";
    ...
    if (idx >= 0) {
      ...
      base = lastSlash > 0 ? pathname.slice(0, lastSlash) : "";
    }
