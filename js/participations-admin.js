...
function loadParticipations() {
    fetch(App.apiUrl('Controller/EventParticipationController.php'), {
        method: 'POST',
...
function renderStats() {
    ...
    statsEl.innerHTML = `
        <div style="background:var(--color-surface); border-radius:10px; padding:18px; border:1px solid var(--color-border); text-align:center;">
            <div style="font-size:2rem; font-weight:800; color:var(--color-text, #333333);">${total}</div>
...
function renderParticipations() {
    ...
    tbody.innerHTML = list.map((p, i) => {
        ...
        return `<tr style="border-bottom:1px solid var(--color-border, rgba(0,0,0,0.05));">
            <td style="padding:14px 16px; color:var(--color-text-muted, #64748b); font-size:0.85rem; font-weight:600;">${p.id}</td>
            <td style="padding:14px 16px;">
                <div style="font-weight:700; color:var(--color-text, #333333);">${p.nom || ''} ${p.prenom || ''}</div>
                <div style="font-size:0.82rem; color:var(--color-text-muted, #64748b); margin-top:3px;"><i class="fa-solid fa-envelope" style="margin-right:5px;"></i>${p.email || '—'}</div>
                ${p.telephone ? `<div style="font-size:0.82rem; color:var(--color-text-muted, #64748b); margin-top:2px;"><i class="fa-solid fa-phone" style="margin-right:5px;"></i>${p.telephone}</div>` : ''}
            </td>
            <td style="padding:14px 16px;">
                <div style="font-weight:600; color:var(--color-text, #333333);">${p.universite || '—'}</div>
                <div style="font-size:0.82rem; color:var(--color-text-muted, #64748b); margin-top:2px;">${p.annee_etude || '—'}</div>
            </td>
            <td style="padding:14px 16px;">
                <div style="font-weight:600; color:var(--color-text, #333333);">${p.event_title || '—'}</div>
                <div style="margin-top:5px;">${typeBadge}</div>
                ${p.event_date ? `<div style="font-size:0.82rem; color:var(--color-text-muted, #64748b); margin-top:3px;"><i class="fa-regular fa-calendar" style="margin-right:4px;"></i>${p.event_date}</div>` : ''}
            </td>
            <td style="padding:14px 16px; color:var(--color-text-muted, #94a3b8); font-size:0.88rem; white-space:nowrap;">${p.date_inscription || '—'}</td>
            <td style="padding:14px 16px;">
