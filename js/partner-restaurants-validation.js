document.addEventListener('DOMContentLoaded', function () {
  var restaurantForm = document.getElementById('partner-restaurant-form');
  var mealForm = document.getElementById('partner-meal-form');
  ...
  if (restaurantForm) {
    restaurantForm.addEventListener('submit', function (event) {
      ...
      var ownerField = restaurantForm.querySelector('input[name="id_owner"]');
      ...
      if (!name || name.length < 2 || name.length > 100 || !/^[A-Za-z0-9\u00C0-\u024F\s'-]+$/.test(name)) {
        setError('restaurant_name-error', nameField, 'Nom invalide (2-100, lettres/chiffres/espaces).');
        valid = false;
      }
      ...
      if (!description || description.length < 10 || description.length > 1000) {
        setError('restaurant_description-error', descriptionField, 'Description obligatoire (10-1000).');
        valid = false;
      }
  ...
  if (mealForm) {
    ...
    function syncPriceField() {
      if (!pricingModeField || !priceField) return;
      if (pricingModeField.value === 'free') {
        priceField.value = '0';
    ...
    if (pricingModeField) {
      pricingModeField.addEventListener('change', function () {
        syncPriceField();
    ...
    mealForm.addEventListener('submit', function (event) {
      ...
      var mode = pricingModeField ? pricingModeField.value : '';
      ...
      if (!name || name.length < 2 || name.length > 100 || !/^[A-Za-z0-9\u00C0-\u024F\s'-]+$/.test(name)) {
        setError('meal_name-error', nameField, 'Nom invalide.');
        valid = false;
      }
      ...
      if (!ingredients || ingredients.length < 2 || ingredients.length > 1000 || !/^[A-Za-z\u00C0-\u024F\s,'-]+$/u.test(ingredients)) {
        setError('meal_ingredients-error', ingField, 'Ingredients invalides (lettres, virgules).');
        valid = false;
      }
      ...
      if (mode !== 'free' && mode !== 'paid') {
        setError('meal_pricing_mode-error', pricingModeField, 'Mode invalide.');
        valid = false;
      }
      if (mode === 'paid') {
        if (!/^\d+(\.\d{1,2})?$/.test(price) || parseFloat(price) <= 0 || parseFloat(price) > 500) {
