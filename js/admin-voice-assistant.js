const AdminVoiceAssistant = {
  ...
  async handlePendingIntent(text) {
    ...
    if (pending.type === "block_user") {
      ...
      await this.updateUserStatusLocal(user.id, "banned", `Utilisateur banni: ${user.name}`);
      this.clearPendingIntent();
    ...
    if (pending.type === "unblock_user") {
      ...
      await this.updateUserStatusLocal(user.id, "active", `Utilisateur reactive: ${user.name}`);
      this.clearPendingIntent();
    ...
    if (pending.type === "open_user_profile_target") {
      ...
      setTimeout(() => {
        window.location.href = App.apiUrl(`View/BackOffice/admin/user-detail.php?id=${encodeURIComponent(user.id)}`);
      }, 250);
      return true;
    }
  ...
  executeNavigationOnly(text) {
    ...
    const navCommands = [
      { words: ["utilisateur", "utilisateurs", "user", "users"], url: "View/BackOffice/admin/users.php", message: "J ouvre la page Utilisateurs." },
      { words: ["partenaire", "partenaires"], url: "View/BackOffice/admin/partners.php", message: "J ouvre la page Partenaires." },
      { words: ["categorie", "categories"], url: "admin/categorie.php", message: "J ouvre la page Categories." },
      { words: ["offre", "offres"], url: "admin/offers.php", message: "J ouvre la page Offres." },
      { words: ["evenement", "evenements", "event", "events"], url: "View/BackOffice/admin/events.php", message: "J ouvre la page Evenements." },
      { words: ["log", "logs", "activite"], url: "View/BackOffice/admin/logs.php", message: "J ouvre la page Logs." },
      { words: ["dashboard", "vue globale", "accueil"], url: "View/BackOffice/admin/dashboard.php", message: "Retour au dashboard." },
    ];
    ...
    for (const cmd of navCommands) {
      if (cmd.words.some((word) => normalized.includes(word))) {
        ...
        setTimeout(() => {
          window.location.href = App.apiUrl(cmd.url);
        }, 250);
        return true;
      }
    }
  ...
  async executeGeminiAction(cmd, originalText = "") {
    ...
    if (action === "block_user" || action === "unblock_user") {
      ...
      await this.updateUserStatusLocal(user.id, nextStatus, `Utilisateur ${nextStatus === "banned" ? "banni" : "reactive"}: ${user.name}`);
      this.respond(
        nextStatus === "banned"
  ...
  async handleAdminOfferSearchIntent(text) {
    ...
    if (!this.isAdminOffersPage()) {
      ...
      setTimeout(() => {
        window.location.href = App.apiUrl('admin/offers.php');
      }, 250);
      return true;
    }
  ...
  async updateUserStatusLocal(id, status, logMessage) {
    const response = await fetch(App.apiUrl('Controller/UserController.php'), {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
    ...
    if (!response.ok || payload?.success === false) {
      const msg = payload?.message || "Mise a jour du statut impossible.";
      throw new Error(msg);
    }
