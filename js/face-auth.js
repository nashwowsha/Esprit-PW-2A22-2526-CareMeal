...
const FaceAuth = {
  ...
  _regPoses: [
    ...
    { name: 'Tournez lentement à gauche', icon: '⬅️', check: (a) => a.yaw > 22, count: 3 },
    { name: 'Tournez lentement à droite', icon: '➡️', check: (a) => a.yaw < -22, count: 3 },
    { name: 'Levez la tête', icon: '⬆️', check: (a) => a.pitch < -15, count: 2 },
    { name: 'Baissez la tête', icon: '⬇️', check: (a) => a.pitch > 15, count: 2 },
  ],

  // Phase 3 — Flash poses (subset with rotations)
  _flashPoses: [
    { name: 'Fixez l\'écran', icon: '🎯', check: (a) => Math.abs(a.yaw) < 12 && Math.abs(a.pitch) < 14, count: 2 },
    { name: 'Tournez vers la gauche', icon: '⬅️', check: (a) => a.yaw > 18, count: 1 },
    { name: 'Tournez vers la droite', icon: '➡️', check: (a) => a.yaw < -18, count: 1 },
  ],
  ...
  async _doFaceLogin(descriptors, statusEl) {
    try {
      ...
      if (result.success) {
        ...
        setTimeout(() => {
          this.closeModal();
          const paths = { student: '/projet2a22/View/FrontOffice/feed.php', partner: '/projet2a22/View/FrontOffice/feed.php', admin: '/projet2a22/View/BackOffice/admin/dashboard.php' };
