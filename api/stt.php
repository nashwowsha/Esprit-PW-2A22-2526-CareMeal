...
function stt_send_json(int $status, array $payload): void
{
...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    stt_send_json(405, ['error' => 'Method not allowed']);
}

if (!isset($_FILES['audio']) || !is_array($_FILES['audio'])) {
    stt_send_json(422, ['error' => 'Audio file is required']);
}

$audio = $_FILES['audio'];
if (($audio['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_OK) {
    stt_send_json(422, ['error' => 'Audio upload failed']);
}

$tmpPath = (string)($audio['tmp_name'] ?? '');
if ($tmpPath === '' || !is_uploaded_file($tmpPath)) {
    stt_send_json(422, ['error' => 'Invalid uploaded file']);
}

$apiKey = trim((string)(Env::get('GROQ_API_KEY', '') ?? ''));
if ($apiKey === '') {
    stt_send_json(500, ['error' => 'GROQ_API_KEY missing in .env']);
}
...
$language = trim((string)($_POST['language'] ?? Env::get('GROQ_STT_LANGUAGE', 'fr') ?? 'fr'));
if ($language === '') {
    $language = 'fr';
}
...
if ($response === false) {
    stt_send_json(502, ['error' => 'Groq STT request failed', 'details' => $curlErr]);
}

$json = json_decode($response, true);
if ($httpCode >= 400) {
    $msg = (string)($json['error']['message'] ?? 'Groq STT error');
    stt_send_json($httpCode, ['error' => $msg]);
}
